...
#' @importFrom CompQuadForm liu davies imhof
...
KNN.overall.test = function(phenotype, genotype, test.index, input.kernel = NULL,
  ...
  p.value = davies(q = 0, lambda = eigenvalues)$Qq
  if (p.value==0){
    p.value = imhof(q = 0, lambda = eigenvalues, epsabs = 1e-12, epsrel = 1e-12, limit = 1e6)$Qq
    p.value = abs(p.value)
  }
...
KNN.ind.test = function(phenotype, genotype, test.index, input.kernel = NULL,
  ...
  p.value = davies(q=theta.full[test.index], lambda=eigenvalues)$Qq
  if (p.value==0){
    p.value = imhof(q=theta.full[test.index], lambda=eigenvalues, epsabs = 1e-12, epsrel = 1e-12, limit = 1e6)$Qq
    p.value = abs(p.value)
  }
...
#' @param input.kernel List specifying the kernel function(s) for each genotype matrix.
#' @param theta Estimated variance components.
#' @param phenotype.covariance Estimated residual covariance matrix Phi of phenotypes (d x d).
#' @param KNN.type Type of KNN model ('KNN', 'LMM') (default 'KNN').
#' @param phenotype.type Phenotype covariance structure for the genetic random effects ('Homo' or 'Heter') (default 'Heter'). Must match the value used in estimation.
...
MVKNN.predict = function(phenotype, genotype, theta, phenotype.covariance, input.kernel = NULL,
                         KNN.type='KNN', phenotype.type='Heter', beta=NULL,
                         fixed.effects.train=NULL, fixed.effects.test=NULL) {
  ...
  # Trait covariance structure Sigma_p for the genetic random effects.
  # This must match the structure used during estimation (see MVKNN / fit.mvknn.internal).
  ...
  # Vectorize the phenotype by stacking columns (trait-major), matching vec(Y) in the paper
  # and the convention used in estimation: vec(Y) = [y_1; ...; y_d].
  ...
  # Get variance components on the full (train + test) sample
  variance.component.list = KNN2LMM(genotype=genotype, input.kernel=input.kernel, KNN.type=KNN.type)
  n.variance.component = length(variance.component.list)

  # Build the multivariate variance components with trait-major Kronecker products, matching
  # the paper and the estimation step exactly:
  #   residual : Phi     %x% I_N    (component 1)
  #   genetic  : Sigma_p %x% K_k    (components >= 2)
  variance.component.list[[1]] = kronecker(phenotype.covariance, diag(total.sample.size))
  ...
  # Total model covariance over all samples:
  #   V = theta_1 (Phi %x% I_N) + sum_{k>=2} theta_k (Sigma_p %x% K_k)
  # Using all variance components (including the residual) keeps prediction consistent with the
  # fitted model. The residual block is sample-diagonal, so it contributes nothing to the
  # test-vs-train cross-covariance, but it must enter the training covariance.
  full.covariance = Reduce(`+`, mapply(`*`, variance.component.list, theta, SIMPLIFY=FALSE))

  # Trait-major index sets. For trait t (0-based) and sample i, position = t * total.sample.size + i.
  # Within each trait block the first training.sample.size rows are training samples and the
  # remaining test.sample.size rows are testing samples.
  trait.offsets = (0:(n.traits - 1)) * total.sample.size
  train.index = as.vector(outer(1:training.sample.size, trait.offsets, "+"))

  if (test.sample.size == 0) {
    n.predict = training.sample.size
    predict.index = train.index
    fixed.effect.predict.vector = as.vector(fixed.effects.train %*% beta)
  } else {
    n.predict = test.sample.size
    predict.index = as.vector(outer((training.sample.size + 1):total.sample.size, trait.offsets, "+"))
    fixed.effect.predict.vector = as.vector(fixed.effects.test %*% beta)
  }

  covariance.train = full.covariance[train.index, train.index]
  covariance.test.train = full.covariance[predict.index, train.index]
  fixed.effect.train.vector = as.vector(fixed.effects.train %*% beta)

  # BLUP prediction
  phenotype.predicted.vector = fixed.effect.predict.vector +
    covariance.test.train %*% solve(covariance.train) %*%
    (phenotype.vectorized - fixed.effect.train.vector)

  # Convert the trait-major predicted vector back to an N.predict x d matrix
  phenotype.predicted = matrix(phenotype.predicted.vector, nrow=n.predict, ncol=n.traits)
...
MVKNN = function(phenotype, genotype, input.kernel = NULL, MINQUE.type='MINQUE0', KNN.type='KNN',
  ...
  # Initialize theta. The first component is the residual (Phi %x% I_N); its variance component
  # (alpha_0) is fixed at 1 throughout, because Phi has already been estimated in Step 2.
  if (is.null(weight)) {
  ...
  for (iteration in 1:max.iter) {
    ...
    # Update theta. Keep the residual variance component (alpha_0) fixed at 1, following the
    # iterative-MINQUE scheme in the paper (Phi is pre-estimated, so its component is not re-scaled).
    covariance.matrix = solve(covariance.matrix)
    theta = as.numeric(covariance.matrix %*% u.matrix)
    theta[1] = 1
...
MVKNN.overall.test = function(phenotype, genotype, test.index=NULL, input.kernel = NULL,
  ...
  p.value = davies(q = test.mean, lambda = eigenvalues)$Qq
  if (p.value==0){
    p.value = imhof(q = test.mean, lambda = eigenvalues, epsabs = 1e-12, epsrel = 1e-12, limit = 1e6)$Qq
    p.value = abs(p.value)
  }
